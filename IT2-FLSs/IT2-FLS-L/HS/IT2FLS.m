...
%                 layer.parameterList(i) = 'l'+string(i);
%             end
...
%             cmfs = repmat(cmfs,1,r);
            ...
            zl = []; zu = [];
            for i=1:c
            [z_l, z_u] = feval(TRMethod,fs_u(:,i),fs_l(:,i),cmfs(:,i),cmfs(:,i),nRule,1);
             zl = [zl z_l];
             zu = [zu z_u];
            end
            Z = [(zu+zl)./2;zl;zu];        
...
%                         i1 = find(MFParams==double(indexlist{i}));
                 ...
                 if iscell(lp{end})
                    ...
                    if lp{1} == 'pd'
                        ...
                        r2 = r_2;
                        layer.parameterList(i,:) = [r1 r2 zeros(1,0)];
                    else
...
%                         layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params = repmat(MFParams,1,size(layer.parameterList,2)-1);
                        if (~iscell(layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params)&layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params~=0)
