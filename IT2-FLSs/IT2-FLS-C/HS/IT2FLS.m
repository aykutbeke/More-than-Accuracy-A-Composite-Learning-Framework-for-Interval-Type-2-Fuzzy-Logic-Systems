...
%             end
            ...
            l_p = layer.parameterList;
            layer.parameterList = l_p;
            for i=1:r
...
%             fss_u=fs_u./(sum(mfs_u,2)+eps);
%             fss_l=fs_l./(sum(mfs_l,2)+eps);
            cmfs=[];
            ...
            layer = getConsequents(layer,cmfs);
           
            [z_l, z_u] = feval(TRMethod,fs_u,fs_l,cmfs,cmfs,nRule,c);
            Z = [(z_u+z_l)./2;z_l;z_u];     

%             consequentMFParams=repmat(consequentMFParams,length(input(k,:)),1);
%             Z = (fs'*consequentMFParams)./sum(fs);
%             y=[];
%             for k=1:r
%                 layer = updateLearnableMFParams(layer,k);
%                 membershipDegrees = helper.evalMfTypeHelper2(mfType,input(k,:),mfParams(k,:));
%                 F = reshape(membershipDegrees,[numel(membershipDegrees),1])';
%                 Firing = reshape(F,[2,nRule* length(input(k,:))])';
%                 consequentMFParams = cat(1,layer.t2fis.output.mf.params);
%                 consequentMFParams=repmat(consequentMFParams,length(input(k,:)),1);
%                 [yL,yR] = feval(TRMethod,Firing,consequentMFParams,nRule);
%                 y=[y; (yL+yR)./2];
%             end
%             Z = (y.*(Xi+eps))./(Xi + eps);
%             Z = y;
