classdef IT2FLS < nnet.layer.Layer
    ...
    methods
        ...
        function Z = predict(layer,Xi)
            ...
            i_t = [i_t;ones(c,r)';abs(i_t);ones(c,r)'];
            fs = prod(mfs,2);
%             fs = min(mfs')';
            fs_u = fs(1:2:end,:); 
            fs_l = fs(2:2:end,:); 
...
%             cmfs = cmfs*reshape(horzcat(input,ones(1,c)),[c,2])';
%             cmfs = repmat(cmfs,1,r);
            cmfs = cmfs*i_t;
            zl = []; zu = [];
            for i=1:c
            cms = cmfs(:,i);
            cmf_l = cms(1:nRule); 
            cmf_u = cms(1+nRule:end);
            [z_l, z_u] = feval(TRMethod,fs_u(:,i),fs_l(:,i),cmf_u,cmf_l,nRule,1);
             zl = [zl z_l];
             zu = [zu z_u];
            end
            Z = [(zu+zl)./2;zl;zu];         
        ...
        indexlist={};
        n_i = size(c,2)*2+1;
        layer.parameterList = zeros(size(layer.t2fis.learnableParameters,2),n_i);
%         t2fis=layer.t2fis;
...
%                 c = c(c~=c(1));
            elseif mf == 'pd'
                ...
                s=s(2:end);
%                 s = s(s~=s(1));
            elseif n_i>2 && lp{end-1}=="o"
                m = initializeMemberShips(layer,i);
                for j=1:(n_i-1)
                    member(j)=m;
                end
            else
...
%                         i2 = find(MFParams2==double(indexlist{i}{2}));
                        layer.mask{end+1} = {1,1};
                        MFParams1 = memberList{i};
%                         MFParams2 = memberList{i};
%                         r1 = rand(numberofChannels,1)*0.01+memberList{i}{1};
%                         r2 = rand(numberofChannels,1)*0.01+memberList{i}{2};
                        sz=size(layer.parameterList,2)-1;
                        if iscell(memberList{i})
                            r = rand(1,sz)*0.01+cell2mat(memberList{i});
                        else
                            r = rand(1,size(layer.parameterList,2)-1)*0.01+memberList{i};   
                        end
                        r1 = r(1:sz/2);r2=r(1+sz/2:sz);
                        layer.parameterList(i,:) = [r1 r2 0];
                        layer.t2fis.output(str2double(varIndex1)).mf(str2double(MFIndex1)).params = MFParams1;
                        layer.t2fis.output(str2double(varIndex2)).mf(str2double(MFIndex2)).params = MFParams1;
                     else
                        varIndex = lp{2};
                        MFIndex = lp{3};
                        MFParams = layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params;
...
%                         i1 = find(MFParams==double(indexlist{i}));
                        layer.mask{end+1} = 1;
                        MFParams =memberList{i};
                        if iscell(memberList{i})
                            r = rand(1,size(layer.parameterList,2)-1)*0.01+cell2mat(memberList{i});
                        else
                            r = rand(1,size(layer.parameterList,2)-1)*0.01+memberList{i};   
                        end
                      
                        layer.parameterList(i,1:end-1) = [r];
                        layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params = MFParams;
                     end
                end
             else
                 if iscell(lp{end})
                    ...
                    r_1=rand(numberofChannels,1)*0.01;
                    r_2=rand(numberofChannels,1)*0.01;
                    if lp{1} == 'pd'
                        r1 = r_1+memberList{i}{1};
                        r2 = r_2;
                        layer.parameterList(i,:) = [r1 r2 n_i-2];
                    else
                        r1 = r_2+memberList{i}{1};
                        r2 = r_2+memberList{i}{2};
                        layer.parameterList(i,:) = [r1 r2 n_i-2];
                    end
                    layer.t2fis.input(str2double(varIndex1)).mf(str2double(MFIndex11),str2double(MFIndex21)).params=MFParams1;
                    layer.t2fis.input(str2double(varIndex2)).mf(str2double(MFIndex12),str2double(MFIndex22)).params=MFParams2;
...
%                         k1=str2double(MFIndex1);k2=str2double(MFIndex2);
                        MFParams = layer.t2fis.output(str2double(varIndex1)).mf(str2double(MFIndex1)).params;
                        MFParams2 = layer.t2fis.output(str2double(varIndex2)).mf(str2double(MFIndex2)).params;
                        sz=size(layer.parameterList(i,1:end-1),2);
                        if iscell(MFParams)
                            MFParams = (cell2mat(MFParams).*(layer.parameterList(i,1:end-1)+eps)+(layer.parameterList(i,1:end-1)+eps));  
                        else
                            MFParams = MFParams.*(layer.parameterList(i,1:end-1)+eps)./(MFParams+eps);
                        end
%                         MF1 = MFParams;
%                         MFParams2(layer.mask{i}{2}) = layer.parameterList(i,2);
                        if (~iscell(layer.t2fis.output(str2double(varIndex1)).mf(str2double(MFIndex1)).params))
                            layer.t2fis.output(str2double(varIndex1)).mf(str2double(MFIndex1)).params =[layer.t2fis.output(str2double(varIndex1)).mf(str2double(MFIndex1)).params -abs(MFParams)];
                            layer.t2fis.output(str2double(varIndex2)).mf(str2double(MFIndex2)).params =[layer.t2fis.output(str2double(varIndex2)).mf(str2double(MFIndex2)).params abs(MFParams)];
                        else
%                             layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params =MFParams;
                            layer.t2fis.output(str2double(varIndex1)).mf(str2double(MFIndex1)).params = MFParams;
                            layer.t2fis.output(str2double(varIndex2)).mf(str2double(MFIndex2)).params = MFParams;
                        end
...
%                         k=str2double(MFIndex);
                        ...
                        if lp{1} == 'delta'
                            ...
                            r1=layer.parameterList(i,1:end-1);
                            MFParams = r1;
                        end
%                         layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params = repmat(MFParams,1,size(layer.parameterList,2)-1);
                        if (~iscell(layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params))
                            layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params =[layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params MFParams];
                        else
                            layer.t2fis.output(str2double(varIndex)).mf(str2double(MFIndex)).params =MFParams;
                        end
                    end
                    ...
                    if iscell(lp{end})
                        ...
                        if  lp{1} == 'pd'
                            MFParams1(layer.mask{i}{1}) = (layer.parameterList(i,1)+abs(layer.parameterList(i,2)));
                            MFParams2(layer.mask{i}{2}) = (layer.parameterList(i,1)-abs(layer.parameterList(i,2)));
%                             MFParams1(layer.mask{i}{1}) = (layer.parameterList(i,1));
%                             MFParams2(layer.mask{i}{2}) = (layer.parameterList(i,1)*sigmoid(layer.parameterList(i,2)));
%                             MFParams1(layer.mask{i}{1}) = (layer.parameterList(i,1)+(layer.parameterList(i,2))).^2;
%                             MFParams2(layer.mask{i}{2}) = (layer.parameterList(i,1)-(layer.parameterList(i,2))).^2;
                            layer.t2fis.input(str2double(varIndex1)).mf(str2double(MFIndex11),str2double(MFIndex21)).params = MFParams1;
                            layer.t2fis.input(str2double(varIndex2)).mf(str2double(MFIndex12),str2double(MFIndex22)).params = MFParams2;
                        else
